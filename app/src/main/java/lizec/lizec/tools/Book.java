...
public class Book {
    ...
    public Page getPage(int index){
        System.out.println("在Book类getPage函数中，URL："+catalogUrls[index]);
        return new Page(catalogUrls[index]);
    }
    ...
    private String[] getCatalogURLStrings(Elements catalog){
        ...
        for(int i=0;i<catalog.size();i++){
            ...
            URLStrings[i]=g_url + e.attr("href");
        }
        return URLStrings;
    }
    ...
    private void initCatalog(Document doc){
        //获得目录相关信息
        Elements list=doc.getElementsByTag("dl");
        //查询a标签获得所有连接
